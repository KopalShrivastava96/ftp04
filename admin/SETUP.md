...
## Pre Nov-20th
  ...
  * Create Git repository for the 10 teams
    ...
    * `git clone --bare git@github.com:HexaInnovLab/ftp-fork-me.git ftp04`
    * `cd ftp04`
    ...
    * For each team, nn = 01 to 10
      ...
      * cd ftpnn
      * For windows
        * use VS Code, find/replace in files to do the following: replacing ftp04 -> ftpnn, FTP04 -> FTPnn
        * Rename directory restservice/leavemanager/src/main/java/com/hexaware/ftp04/ to restservice/leavemanager/src/main/java/com/hexaware/ftpnn/
      * For Mac OS
        * NOTE: replace nn with the team-id in the following three commands
        * `find . -type f -not -path '*/\.*' -not -path "*/node_modules/*" -exec sed -i '' -e 's/ftp04/ftpnn/g' {} \; -exec sed -i '' -e 's/FTP04/FTPnn/g' {} \;`
        * `mv restservice/leavemanager/src/main/java/com/hexaware/ftp04/ restservice/leavemanager/src/main/java/com/hexaware/ftpnn/`
  ...
  * Databases - 3 per team
    * Integration and Staging instances
      ...
      * 
    * `CREATE DATABASE FTP04;` -- XX from 01 to 10
    * `CREATE USER 'FTP04'@'%' IDENTIFIED BY 'FTP04';`
    * `GRANT ALL ON FTP04.* TO 'FTP04'@'%';`
    * `select Host, User, Password from mysql.user order by user;`
    * `select * from mysql.db order by Db;`
  * Jenkins jobs - 3 per team
    * FTP04-10-UNIT
    * FTP04-30-INTEGRATION
    * FTP04-50-STAGING
    * For each team, nn = 01 to 10
      * Create a tab FTPnn
